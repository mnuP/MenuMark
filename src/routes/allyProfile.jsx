          ...
          <Row xs={1} md={4} className="g-4 itemsPage">
            {items.map((item) => (
                <div className='ItemsInside'>
                  <Col>
                    <Card className={"cards"} key={item.id} onClick={()=>{navigate("Modif", {state:{item}})}}>
                      <Card.Img className={"cardsImageSec"} variant="top" src={item.photo} />
                      <Card.Body className={"cardsBody"}>
                        <Card.Title key={item.title}>{item.title}</Card.Title>
                        <Card.Text style={{fontSize: 18}}> Ultima vez actualizado: {item.timeStamp.toDate().toLocaleString()} </Card.Text>    
                      </Card.Body>
                    </Card>
                  </Col>
                </div>
